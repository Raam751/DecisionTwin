 ...
 * condense-resume
 *
 * Takes the raw text extracted from a candidate resume and asks the model for a
 * much shorter, factual list the reviewer can confirm: job titles, companies,
 * responsibilities, notable achievements, and a short education summary. Each
 * returned line is short and standalone so it maps cleanly to a numbered line.
 *
 * Required secrets, shared with generate-evidence, generate-criteria and
...
function buildPrompt(body: RequestBody): string {
  return [
    "You condense a candidate resume into a much shorter, factual list a reviewer can check against hiring criteria.",
    "",
    "RESUME TEXT",
    body.resumeText.trim(),
    "",
    "RULES",
    "1. Keep only the substance: job titles, companies, responsibilities, notable achievements, and a short education summary.",
    "2. Drop contact details, page headers, dates lists, verbose bullet points, repeated content, and anything that does not describe the candidate's professional background.",
    "3. Write each line as one short, standalone sentence or phrase that reads clearly on its own.",
    "4. Put the candidate's name on the first line when it appears in the text.",
    "5. Return between 8 and 15 lines. Never more than 15, never fewer than 8.",
    "6. Do not invent facts. Do not change the meaning of anything in the resume.",
    "7. Keep facts even when they are long lists; compress them into fewer, denser lines instead of dropping them.",
    "",
    "Return only JSON in this shape:",
    '{"lines":["Priya Menon, Senior Platform Engineer","Owned the partner-facing REST API end to end"]}',
  ].join("\n");
}
...
const SYSTEM_PROMPT =
  "You return only valid JSON. You condense a candidate resume into a short factual list, you never invent facts, and you never change the meaning of the source.";
...
/**
 * Keeps only usable lines: trimmed, deduplicated, capped at 15. A reply with
 * fewer than two usable lines is treated as unusable so the caller falls back
 * to the raw extracted lines.
 */
function shapeLines(raw: unknown): string[] {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    parsed = result.parsed as { lines?: unknown };
  } catch (error) {
  ...
  if (lines.length < 2) {
    return json({ error: "model returned fewer than two usable lines" }, 502);
  }
